    ...
    public function checkCompatibility() {
        ...
        if (!is_admin()) {
            ...
            if (HMW_Classes_Tools::isThemeActive('Avada')) {
                add_filter('template_directory_uri', array(HMW_Classes_ObjController::getClass('HMW_Models_Rewrite'), 'find_replace_url'), PHP_INT_MAX);
            }

            //Compatibility with Squirrly SEO
            if (HMW_Classes_Tools::isPluginActive('squirrly-seo/squirrly.php')) {
                add_filter('sq_buffer', array(HMW_Classes_ObjController::getClass('HMW_Models_Rewrite'), 'find_replace'), PHP_INT_MAX);
            }
	        ...
	        if ( HMW_Classes_Tools::isPluginActive( 'wp-asset-clean-up/wpacu.php' ) || HMW_Classes_Tools::isPluginActive( 'wp-asset-clean-up-pro/wpacu.php' ) ) {
		        ...
		        add_filter('wpacu_html_source', array(HMW_Classes_ObjController::getClass('HMW_Models_Rewrite'), 'find_replace'), PHP_INT_MAX);
		        return;
	        }
    ...
    public static function getAlerts() {
        ...
        if (is_admin() || is_network_admin()) {
            if (HMW_Classes_Tools::isPluginActive('cdn-enabler/cdn-enabler.php')) {
                ...
                if (isset($_SERVER["REQUEST_URI"])) {
                    if (admin_url('options-general.php?page=cdn_enabler', 'relative') == $_SERVER['REQUEST_URI']) {
                        HMW_Classes_Error::setError(sprintf(__("CDN Enabler detected! Learn how to configure it with Hide My WP %sClick here%s", _HMW_PLUGIN_NAME_), '<a href="http://hidemywpghost.com/article/hide-my-wp-and-cdn-enabler/" target="_blank">', '</a>'), 'error');
                    }
                }
            }
            ...
            if (HMW_Classes_Tools::isInmotion()) {
                HMW_Classes_Error::setError(sprintf(__('Inmotion detected. %sPlease read how to make the plugin compatible with Inmotion Nginx Cache%s', _HMW_PLUGIN_NAME_), '<a href="http://hidemywpghost.com/article/hide-my-wp-pro-compatible-with-inmotion-wordpress-hosting/" target="_blank">', '</a>'));
            }
