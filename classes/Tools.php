...
class HMW_Classes_Tools extends HMW_Classes_FrontController {
    ...
    public static function checkApi($email = null, $redirect_to = '') {
        ...
        if (isset($email) && $email <> '') {
            $args = array('email' => $email, 'url' => home_url(), 'howtolessons' => (int)$howtolessons, 'monitor' => (int)$monitor, 'source' => _HMW_PLUGIN_NAME_);
            $response = self::hmw_remote_get(_HMW_API_SITE_ . '/api/free/token', $args, array('timeout' => 10));
        } elseif (self::getOption('hmw_token')) {
            $args = array('token' => self::getOption('hmw_token'), 'url' => home_url(), 'howtolessons' => (int)$howtolessons, 'monitor' => (int)$monitor, 'source' => _HMW_PLUGIN_NAME_);
            $response = self::hmw_remote_get(_HMW_API_SITE_ . '/api/free/token', $args, array('timeout' => 10));
        } else {
            return $check;
        }
